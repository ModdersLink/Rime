...
namespace RimeLib.Cmd.Commands.BundleBuilding
{
    ...
    public class AddDdsTextureCommand : Command
    {
        ...
        [CommandArgument(Description = "Whether the texture is streamable. Defaults to 'true'. Use 'false' for a texture shipped self-contained in a mod bundle.", Optional = true)]
        ...
        // The engine's texture pools gate on the group name, and "Default" is not one the game knows,
        // so a texture under a new name that has no mounted original to copy the group from is never
        // uploaded by the pool.
        [CommandArgument(Description = "TextureGroup override, e.g. 'AtlasTextureGro'. Defaults to the mounted original's group, or 'Default'.", Optional = true)]
