...
{
	[ContainerType(4, 12)]
	public partial class UILevelSpecificPageHeader :
		EbxSerializable
	{
		[ObservableProperty]
		[property: ContainerField(0), LayoutImmutable, JsonProperty(Order = 0)]
		private string _LevelNameSID = string.Empty;
		...
		public override void Serialize(RimeWriter p_Writer, IEbxWriter p_EbxWriter)
		{
			...
			p_Writer.Write(p_EbxWriter.WriteString(LevelNameSID));
			p_Writer.Write(p_EbxWriter.WriteString(Header));
			p_Writer.Write(p_EbxWriter.WriteString(SubHeader));
		}
	}
}
