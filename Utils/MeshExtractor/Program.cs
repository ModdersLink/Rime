    ...
    class Program
    {
        ...
        private static async void DumpFiles(Options p_Options, IEngineMounter p_Mounter)
        {
            ...
            foreach (var s_Resource in s_Resources)
            {
                ...
                //if (s_Name != "props/streetprops/barracks_01/barracks_01_mesh")
                //    continue;
        ...
        private static void ParseRigidMeshBlender(IEngineMounter p_Mounter, MeshSetLayout p_Layout, Options p_Options = null)
        {
            ...
            //using TextWriter s_Writer = new StreamWriter("model.py");


            //s_Writer.WriteLine("# Generated by RimeLib (c) 2020");
            //s_Writer.WriteLine("# Exported by Rime (c) kiwidoggie productions 2017-2020, models copyright respective ip owners, not for redistribution");
            ...
            for (var s_LodIndex = 0; s_LodIndex < p_Layout.LodCount; ++s_LodIndex)
            {
                ...
                using (var s_IndexReader = new RimeReader(new MemoryStream(s_PrimitiveChunkData)))
                {
                    ...
                    foreach (var s_SubsetPair in s_Ret)
                    {
                        ...
                        var s_VertexBuilder = new StringBuilder();
                        for (var i = 0; i < s_SubsetValue.Count; ++i)
                        {
                            var s_Vert = s_SubsetValue[i];

                            dynamic s_Pos = s_Vert[VertexElementUsage.VertexElementUsage_Pos];

                            s_VertexBuilder.AppendLine($"Vector(({s_Pos.Item1}, {s_Pos.Item2}, {s_Pos.Item3})){(i != s_SubsetValue.Count ? "," : "")}");
                        }
                        ...
                        for (var s_PrimitiveIndex = 0; s_PrimitiveIndex < s_PrimitiveList.Count; ++s_PrimitiveIndex)
                        {
                            ...
                            s_PrimitiveWriter.AppendLine($"[{s_FirstIndex}, {s_SecondIndex}, {s_ThirdIndex}]{(s_PrimitiveIndex != s_PrimitiveList.Count ? "," : "")}");
                        }

                        var s_ObjectName = p_Layout.ShortName.Object;
        ...
        private static void DumpMesh<T>(IEngineMounter p_Mounter, Options p_Options, string p_FilePath, IMountedObject<T> p_Object) where T : IObjectVariant
        {
            ...
            ParseRigidMeshBlender(p_Mounter, s_MeshSetLayout, p_Options);
            return;
