...
using RimeLib.Serialization.Frostbite2013_2.Json;
...
public class DatabasePartition : DatabasePartitionBase
{
    ...
    public Dictionary<DataContainerId, DataContainerBase> InstanceMap { get; set; } = new();

    [JsonIgnore]
    public override IEnumerable<DataContainerBase> Instances => InstanceMap.Values;

    [JsonIgnore]
    public override DataContainerBase PrimaryInstance => InstanceMap[new DataContainerId.Guid(PrimaryInstanceGuid)];
        
    // [JsonIgnore]
    // public DataContainerBase PrimaryInstanceCtr => InstanceMap[new DataContainerId.Guid(PrimaryInstanceGuid)];
