...
namespace RimeLib.Texture.Frostbite.DDS
{
    public static class DDSExporter
    {
        public static void WriteTextureToStream( RimeWriter p_Stream, ITexture p_Texture )
        {
            ...
            if ( !TextureUtils.ComputePitch( p_Texture.Format, ( uint )p_Texture.Width, ( uint )p_Texture.Height, out s_RowPitch, out s_SlicePitch ) )
            ...
            if ( DDSUtils.s_DDSFormatMap.TryGetValue( p_Texture.Format, out var s_Format ) )
            {
            ...
            {
                if ( !DDSUtils.s_DDSDXFormatMap.TryGetValue( p_Texture.Format, out var s_DXGIFormat ) )
