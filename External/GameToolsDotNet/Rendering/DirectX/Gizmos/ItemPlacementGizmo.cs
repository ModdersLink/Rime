using GameToolsDotNet.Rendering.DirectX.Gizmos.Polygons;
...
namespace GameToolsDotNet.Rendering.DirectX.Gizmos
{
    ...
    public class ItemPlacementGizmo : IRenderable
    {
        ...
        public Vector3 BoundingBoxMin { get; protected set; }
        public Vector3 BoundingBoxMax { get; protected set; }
        ...
        public ItemPlacementGizmo(Vector3 position, Vector3 rotation, Vector3 boundingBoxMin, Vector3 boundingBoxMax)
        {
            // Initialize fields.
            ...
            this.boundingBox = new Box(this.BoundingBoxMin, this.BoundingBoxMax, Vector3.Zero, Quaternion.Identity);
            ...
            this.ringRadius = largestSideLength + 0.5f;
            ...
            this.rotationArrows[0] = new Arrow(this.arrowHeight, new Vector3(0f, 0f, (this.ringRadius + 0.35f)), Quaternion.RotationYawPitchRoll(0, MathUtil.DegreesToRadians(180), 0f));
            this.rotationArrows[0].Visible = false;
            this.rotationArrows[0].Color = new Color4(0xFFFF0000);

            this.rotationArrows[1] = new Arrow(this.arrowHeight, new Vector3(this.ringRadius + 0.35f, 0f, 0f), Quaternion.RotationYawPitchRoll(MathUtil.DegreesToRadians(270), 0f, MathUtil.DegreesToRadians(90)));
            this.rotationArrows[1].Visible = false;
            this.rotationArrows[1].Color = new Color4(0xFF00FF00);

            this.rotationArrows[2] = new Arrow(this.arrowHeight, new Vector3(0f, this.ringRadius + 0.35f, 0f), Quaternion.RotationYawPitchRoll(MathUtil.DegreesToRadians(90), MathUtil.DegreesToRadians(90), 0f));
        ...
        private void UpdateColors()
        {
            ...
            if (this.isFocused == true)
                this.boundingBox.LineColor = this.boxCornerHighlightColor;
            else
                this.boundingBox.LineColor = this.boxCornerColor;
        }
        ...
        public bool DrawFrame(RenderManager manager, params object[] args)
        {
            ...
            if (this.isFocused == true)
            {
                ...
                if (this.FocusedRotationAxis == RotationAxis.None)
                {
                    ...
                    for (int i = 0; i < this.rotationArrows.Length; i++)
                    {
                        ...
                        this.rotationArrows[i].Style = this.rotationArrows[i].DoPickingTest(manager, pickingRay, null, out float distance, out _) == true ? PolygonDrawStyle.Solid : PolygonDrawStyle.Outline;
                    }
                }
                else
                {
            ...
            // Disable the depth stencil test so the gizmo mesh bleeds through level geometry.
            manager.Device.ImmediateContext.OutputMerger.SetDepthStencilState(manager.HighlightDepthStencilState, 0);
        ...
        {
            if (this.gizmoMesh != null)
                this.gizmoMesh.CleanupGraphics(manager);
        }
        ...
        public bool HandleInput(RenderManager manager, out Vector3 translationChange, out Vector3 rotationChange)
        {
            ...
            if (manager.InputManager.ButtonPressed(InputAction.RightClick) == true)
            {
                ...
                for (int i = 0; i < this.rotationArrows.Length; i++)
                {
                    ...
                    if (this.rotationArrows[i].DoPickingTest(manager, pickingRay, null, out float distance, out object context) == true)
                    {
