...
public class EngineMounter : IEngineMounter
{
    ...
    protected List<PackageManifest> m_Packages = new ();
    
    protected List<SuperbundleEntry> m_Superbundles = new ();


    protected CatalogRegistry m_CatalogRegistry = new();
    
    protected Dictionary<GUID, Catalog> m_InstallCatalogs = new();
    protected Dictionary<string, GUID> m_InstallCatalogNames = new();
    protected Dictionary<string, GUID> m_SuperBundleToCatalog = new(); //TODO: remove this map
    ...
    {
        return EngineType.Frostbite2014_4;
    }
    ...
    protected void ReadLayout()
    {
        ...
        if (s_ContentManifest.InstallManifest != null)
        {
            var s_LanguageMap = new Dictionary<string, string>();
            foreach (var s_Language in s_ContentManifest.InstallManifest.Localizations)
            {
                foreach (var s_Superbundle in s_Language.Superbundles)
                {
                    s_LanguageMap.Add(s_Superbundle.ToLowerInvariant(), s_Language.Language);
                }
            }

            var s_InstallChunks = new Dictionary<GUID, Tuple<GUID, string, GUID[]>>();
            foreach (var s_Chunk in s_ContentManifest.InstallManifest.Chunks)
            {
                string s_InstallBundle = s_Chunk.InstallBundle ?? string.Empty;
                s_InstallChunks.Add(s_Chunk.Id, new Tuple<GUID, string, GUID[]>(s_Chunk.Id, s_InstallBundle, s_Chunk.RequiredChunks));
            }
        

            var s_InstallPackages = new Dictionary<string, Tuple<GUID, GUID, string?>>();
            foreach (var s_Chunk in s_ContentManifest.InstallManifest.Chunks)
            {
                var s_BundleId = string.IsNullOrEmpty(s_Chunk.InstallBundle) ? s_Chunk.InstallPackageId : s_Chunk.Id;

                if (!s_InstallChunks.ContainsKey(s_Chunk.InstallPackageId))
                    continue;

                var s_SuperBundle = m_Superbundles
                    .FirstOrDefault(x => s_Chunk.Superbundles.Select(x => x.ToLowerInvariant()).Contains(x.Name));

                if (s_SuperBundle == null)
                    continue;

                string? s_Language = null;
                s_LanguageMap.TryGetValue(s_SuperBundle.Name, out s_Language);
                
                s_InstallPackages.Add(s_SuperBundle.Name, new Tuple<GUID, GUID, string?>(s_Chunk.InstallPackageId, s_BundleId, s_Language));
            }


            foreach (var s_Package in s_InstallPackages)
            {
                if (!s_InstallChunks.TryGetValue(s_Package.Value.Item2, out var s_Chunk))
                    continue;
                
                foreach (var s_RequiredChunk in s_Chunk.Item3)
                {
            ...
            foreach (var s_Chunk in s_ContentManifest.InstallManifest.Chunks)
            {
                //TODO: id or installpackageid?
                //game seems to do [if (s_Chunk.InstallBundle != null) Id else InstallPackageId]
                // but does use them seperatly as well
                // package might be those packages read earlier.
                
                if(s_Chunk.Id != s_Chunk.InstallPackageId)
                    Debugger.Break();

                if (s_Chunk.InstallBundle == null)
                {
                    Debug.WriteLine($"Install chunk '{s_Chunk.Id}' has no name.");
                    continue;
                }
                
                
                m_InstallCatalogNames.Add(s_Chunk.InstallBundle!, s_Chunk.Id);

                var s_CatalogPath = Path.Join(GetMainPackagePath(), s_Chunk.InstallBundle!, "cas.cat");
                
                if (!File.Exists(s_CatalogPath))
                    continue;

                var s_Catalog = new Catalog(s_CatalogPath);
                ...
                {
                    if (m_SuperBundleToCatalog.ContainsKey(s_Superbundle.ToLowerInvariant()))
                        throw new InvalidDataException("Multiple catalogs for single superbundle.");
                    
                    m_SuperBundleToCatalog.Add(s_Superbundle.ToLowerInvariant(), s_Chunk.Id);
                }
    ...
    protected void ProcessChunk(ChunkInfo p_Chunk, SuperbundleEntry p_SbEntry, bool p_Patch)
    {
        ...
        if (p_SbEntry.Cas)
        {
            ...
            // uses sb name to lookup info
            if (m_SuperBundleToCatalog.TryGetValue(p_SbEntry.Name, out var s_CatalogId))
            {

                if (!m_InstallCatalogs.TryGetValue(s_CatalogId, out var s_Catalog))
                    throw new InvalidDataException("Could not find catalog for catalog id. This should never happen");

                s_ChunkEntry = new CasChunkEntry(p_Chunk.Id, p_Chunk.Sha1, [s_Catalog]);
            }
            else
            {
                // not too sure what to do here. game seems to add all catalog hashees to a shared hash map
                // just scan each catalog and get first one?
                var s_Catalogs = m_InstallCatalogs.Values.Where(x => x.ContainsEntry(p_Chunk.Sha1)).ToList();

                if (s_Catalogs.Count == 0)
                    throw new Exception("Catalog chunk not in any catalogs!");

                
                s_ChunkEntry = new CasChunkEntry(p_Chunk.Id, p_Chunk.Sha1, s_Catalogs.ToArray());
       
                if (s_ChunkEntry == null)
                    throw new InvalidDataException("Could not find Catalog for this chunk.");
    ...
    {
        //Catalog? s_ContainedCatalog = null;
        
        //if(m_SuperBundleToCatalog.TryGetValue(p_Bundle.ContainedSuperbundle.Name, out var s_CatalogId))
        //    s_ContainedCatalog = m_InstallCatalogs[s_CatalogId];
        
        //TODO: Delta bundle, and patch reader//
        // Mount all resources.
        foreach (var s_Resource in p_Bundle.Bundle.ResourceEntries)
        {
            //if (!s_ContainedCatalog.ContainsEntry(s_Resource.Hash) && m_CatalogRegistry.TryGetEntry(s_Resource.Hash, out var s_ContainingCat))
            //    throw new Exception($"Containing catalog is diffrent! {s_ContainingCat.FirstEntry.ContainedCatalog.Path}" );
   
            // Create variant.
            var s_Readable = new CatalogReadable(m_CatalogRegistry!, s_Resource.Hash, s_Resource.OriginalSize != s_Resource.Size, s_Resource);
...
#if DEBUG
           // if (!s_ContainedCatalog.ContainsEntry(s_Chunk.Hash) && m_CatalogRegistry.TryGetEntry(s_Chunk.Hash, out var s_ContainingCat))
           //     throw new Exception($"Containing catalog is diffrent! {s_ContainingCat.FirstEntry.ContainedCatalog.Path}" );
#endif
            ...
            // Create variant.
            var s_Readable = new CatalogReadable(m_CatalogRegistry!, s_Chunk.Hash, s_Chunk.Id.HasCompressionFlag(), s_Chunk);
        ...
        {
            //if (!s_ContainedCatalog.ContainsEntry(s_Partition.Hash) && m_CatalogRegistry.TryGetEntry(s_Partition.Hash, out var s_ContainingCat))
            //    throw new Exception($"Containing catalog is diffrent! {s_ContainingCat.FirstEntry.ContainedCatalog.Path}" );
            
            // Create variant.
            var s_Readable = new CatalogReadable(m_CatalogRegistry!, s_Partition.Hash, s_Partition.OriginalSize != s_Partition.Size, s_Partition);
        ...
        {
            //if (!s_ContainedCatalog.TryGetEntry(s_Partition.Hash, out var s_CatEntry) && m_CatalogRegistry.TryGetEntry(s_Partition.Hash, out var s_ContainingCat))
                //throw new Exception($"Containing catalog is diffrent! {s_ContainingCat.FirstEntry.ContainedCatalog.Path}" );

            if (!m_CatalogRegistry.ContainsEntry(s_Partition.Hash))
                continue;
