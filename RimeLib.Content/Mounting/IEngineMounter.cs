...
namespace RimeLib.Content.Mounting
{
    ...
    public interface IEngineMounter : IEngineInterface
    {
        ...
        IEnumerable<(string Name, ResourceType ResourceType)> GetResourcesInBundle(string p_Bundle);
        ...
        /// <summary>
        /// List all the chunks (with their asset name hash) contained in a specific mounted bundle. (Only non cas bundles.)
        /// </summary>
        /// <param name="p_Bundle">The name of the bundle.</param>
        /// <returns>A list of chunk ids.</returns>
        IEnumerable<(GUID Guid, int AssetNameHash)> GetChunksWithHashInBundle(string p_Bundle);
